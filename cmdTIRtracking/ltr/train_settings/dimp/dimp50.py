...
def run(settings):
    ...
    # hsirgb_train = Hsirgb(settings.env.hsirgb_dir, split='train')
    hsifci_train = Hsifci(settings.env.hsifci_dir, split='train')
    # trackingnet_train = TrackingNet(settings.env.trackingnet_dir, set_ids=list(range(2)))
    # coco_train = MSCOCOSeq(settings.env.coco_dir)

    # Validation datasets
    # got10k_val = Got10k(settings.env.got10k_dir, split='votval')
    # hsirgb_val = Hsirgb(settings.env.hsirgb_dir, split='val')
    ...
    dataset_train = sampler.DiMPSampler([hsifci_train], [1],
                                        samples_per_epoch=26000, max_gap=30, num_test_frames=3, num_train_frames=3,
                                        processing=data_processing_train)
    ...
    dataset_val = sampler.DiMPSampler([hsifci_val], [1], samples_per_epoch=5000, max_gap=30,
    ...
    delta_lr=1e-10                    
    # Optimizer
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5*delta_lr},
                            {'params': actor.net.classifier.filter_optimizer.parameters(), 'lr': 5e-4*delta_lr},
                            {'params': actor.net.classifier.feature_extractor.parameters(), 'lr': 5e-5*delta_lr},
                            {'params': actor.net.bb_regressor.parameters()},
                            {'params': actor.net.feature_extractor.parameters(), 'lr': 2e-5*delta_lr}],
                           lr=2e-4*delta_lr)
