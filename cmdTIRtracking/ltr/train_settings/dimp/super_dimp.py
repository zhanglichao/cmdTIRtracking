...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq, Tir
...
def run(settings):
    ...
    # settings.print_stats = ['Loss/loss_bb_ce']
    # Train datasets
    ...
    # hsifci_train = Hsifci(settings.env.hsifci_dir, split='train')
    # rgbt210_train = Rgbt210(settings.env.rgbt210_dir, split='train')
    tir_train = Tir(settings.env.tir_dir, split='train')

    # Validation datasets
    # got10k_val = Got10k(settings.env.got10k_dir, split='votval')
    # hsifci_val = Hsifci(settings.env.hsifci_dir, split='val')
    # rgbt210_val = Rgbt210(settings.env.rgbt210_dir, split='val')
    tir_val = Tir(settings.env.tir_dir, split='val')
    ...
    dataset_train = sampler.DiMPSampler([tir_train], [1],
    ...
    dataset_val = sampler.DiMPSampler([tir_val], [1], samples_per_epoch=5000, max_gap=30,
    ...
    net = dimpnet.dimpnet50(filter_size=settings.target_filter_sz, backbone_pretrained=True, optim_iter=5,
                            ...
                            frozen_backbone_layers=['conv1','layer1', 'layer2','layer3','layer4','fc'])
    ...
    delta_lr=1e-4
    # clf, iou joint
    # change the optimizer to fine-tune the components of the networks. Here is the joint learning.
    optimizer = optim.Adam([{'params': actor.net.classifier.feature_extractor.parameters(), 'lr': 5e-5*delta_lr},
        {'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 5e-5*delta_lr},
        {'params': actor.net.classifier.filter_optimizer.parameters(), 'lr': 5e-4*delta_lr},
        {'params': actor.net.bb_regressor.parameters(), 'lr': 1e-3*delta_lr}],
                                  lr=2e-4*delta_lr)
