...
class DiMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        """
        # Run network
        # train_images: reference branch
        # test_images: test branc
        target_scores, iou_pred = self.net(train_imgs=data['train_images'],
                                            train_imgs_t=data['train_images_t'],
                                           test_imgs=data['test_images'],
                                           test_imgs_t=data['test_images_t'])
...
class KLDiMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        returns:
            ...
            stats  -  dict containing detailed losses
        """
        # Run network RGB+:reference T:test
        ...
        # train_loss = torch.tensor(0.0, device='cuda:0', requires_grad=True)
        # test_loss = torch.tensor(0.0, device='cuda:0', requires_grad=True)
        ...
        # for s in train_feat:
            # mm = list(train_feat[s].size())
            # nn = list(test_feat[s].size())
            # train_loss = torch.sum(((train_feat[s] - train_feat_t[s]).data**2).data).item() / (mm[-1]*mm[-2]*mm[-3]*mm[-4]) + train_loss
            # test_loss = torch.sum(((test_feat[s] - test_feat_t[s]).data**2).data).item() / (nn[-1]*nn[-2]*nn[-3]*nn[-4]) + test_loss
